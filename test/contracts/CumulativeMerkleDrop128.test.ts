...
async function makeDrop(
    ...
    return { hashedElements, leaves, root, proofs: proofs as unknown as string[][] };
}
...
describe('CumulativeMerkleDrop128', function () {
    ...
    it.skip('Benchmark 30000 wallets (merkle tree height 15)', async function () { // if you want to run this test, add verify & verifyAsm to CumulativeMerkleDrop128.sol
    ...
    describe('behave like merkle drop', function () {
        async function makeDropForSomeAccounts(
            ...
            const walletAddressesSalts = await Promise.all(wallets.map(async (w, i) => {
                const address = await w.getAddress();
                // Combine salt and address for the merkle tree
                return salts[i] + address.slice(2).toLowerCase();
            }));
            return {
                ...(await makeDrop(token, drop, walletAddressesSalts, params.amounts, params.deposit)),
                salts,
                wallets,
            };
        }
